...
upload() {
    ...
    for f in ./coog-*
    do
        CREATE_RELEASE=$(curl -L -X POST -H "Accept: application/vnd.github+json" -H "Authorization: Bearer ${GITHUB_TOKEN}" -H "X-GitHub-Api-Version: 2022-11-28" https://api.github.com/repos/coopengo/tryton/releases '{"tag_name":"${v}","target_commitish":"${CI_COMMIT_REF_NAME}","name":"${v}","body":"Coog client for ${v}"}')
